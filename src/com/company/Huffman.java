...
public class Huffman {
    ...
    public Huffman(String text){
        ...
        for (Character t: directory.tableFrequency.keySet()
        ) {
            queue.add(new TreeNode(t, directory.tableFrequency.get(t)));//создаём деревья
        }
        while (queue.size()!=1){
    ...
    private void codeCharacter(TreeNode treeNode, String code){// left = 0; right = 1
        if(treeNode.chr!=0) {
    ...
    public double getDisperse(){
        ...
        for (Character t: codeTable.keySet()
            ...
            value+=Math.pow(getMediumLength()-codeTable.get(t).length(),2);
        }
